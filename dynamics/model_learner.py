...
class _WorldModel(nn.Module):
    ...
    def setup(self):
        ...
        self.decoder = MLP(
            [self.hidden_dims[-1], self.obs_dim],
    ...
    def __call__(
        ...
        zN_dist = self.dynamics_head(
            jnp.concatenate([z, actions], axis=1), training=training
        )
        mu, logvar = (
            zN_dist[:, : self.hidden_dims[-1]],
            zN_dist[:, self.hidden_dims[-1] :],
        )
...
class Learner(object):
    def __init__(
        ...
        **kwargs
        ...
        model_def = WorldModel(
            model_hidden_dims, obs_dim, action_dim, dropout_rate=dropout_rate
        )
