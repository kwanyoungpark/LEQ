...
def sample_step(rng, model, observations, actions):
    sN_dist, r_hat, mask_hat = model(observations, actions)
    means, log_vars = sN_dist
    sN_hat = means
...
class EffEnsembleDynamicModel(nn.Module):
    ...
    def __call__(
        ...
        if not self.output_all:
            ...
            z = jnp.zeros((self.model.num_models, R, C))
            z = z.at[self.elites].set(tmp)
        ...
        if not self.output_all:
            ...
            ensemble_samples = jnp.zeros((E * R, Co))
            ensemble_samples = ensemble_samples.at[idxs].set(result)
            ensemble_samples = ensemble_samples[:N]
            samples = jnp.concatenate(
                [ensemble_samples[:, :-1] + observations, ensemble_samples[:, -1:]],
                axis=1,
            )
            next_obs = samples[..., :-1]
            ...
            samples = jnp.concatenate(
                [
                    ensemble_samples[:, :, :-1] + observations[None],
                    ensemble_samples[:, :, -1:],
                ],
                axis=2,
            )
...
def get_world_model(
    ...
    if True:
        ...
        mu = np.load(
            os.path.join(
                "../OfflineRL-Kit/models/dynamics-ensemble-32/", env_name, "mu.npy"
            )
        )
        std = np.load(
            os.path.join(
                "../OfflineRL-Kit/models/dynamics-ensemble-32/", env_name, "std.npy"
            )
        )
        ckpt = torch.load(
            os.path.join(
                "../OfflineRL-Kit/models/dynamics-ensemble-32/",
                env_name,
                "dynamics.pth",
            )
        )
        ...
        model_def = EnsembleWorldModel(
            40, 32, (200, 200, 200, 200), obs_dim, action_dim, dropout_rate=None
        )
        model_def = EffEnsembleDynamicModel(
            model_def, scaler, reward_scaler, elites, termination_fn
        )
...
class Learner(object):
    def __init__(
        ...
        **kwargs,
        ...
        model = Model.create(
            model_def, inputs=[model_key, observations, actions], tx=optimiser
        )
