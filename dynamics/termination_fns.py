...
def get_termination_fn(task):
    if 'dmc' in task:
        ...
        radius = 0.25
        if 'umaze' in task: center = (0.75, 8.75)
        if 'medium' in task: center = (20.75, 20.75)
        if 'large' in task: center = (32.75, 24.75)
        if 'ultra' in task: center = (52.75, 36.75)
        center = np.array(center)
        center = jax.device_put(center); radius = jax.device_put(radius)
