...
def main(_):
    ...
    if FLAGS.dynamics == 'torch': 
        ...
        if 1 <= FLAGS.seed and FLAGS.seed <= 5:
            print("TESTING SEEDS!")
            model_path = os.path.join('../OfflineRL-Kit/models/dynamics-ensemble/', str(FLAGS.seed), FLAGS.env_name)
        else:
            model_path = os.path.join('../OfflineRL-Kit/models/dynamics-ensemble/', str(1), FLAGS.env_name)
    ...
    if FLAGS.pretrain:
        ...
        agent.target_critic = agent.target_critic.replace(params=agent.critic_pretrain.params)

    video_path = os.path.join(FLAGS.save_dir, 'videos', FLAGS.env_name, str(FLAGS.seed), str(FLAGS.expectile))
    model_path = os.path.join(FLAGS.save_dir, 'models', FLAGS.env_name, str(FLAGS.seed), str(FLAGS.expectile))
    ...
    for i in tqdm.tqdm(range(1, FLAGS.max_steps + 1),
        ...
        if FLAGS.no_rollout:
            ...
            if (i - 1) % FLAGS.rollout_freq == 0:
                ...
                rollout = agent.rollout(rng, data_batch.observations, FLAGS.rollout_length, 1.0)
        ...
        if i % FLAGS.eval_interval == 0:
            eval_stats = evaluate(FLAGS.seed, agent, eval_envs, video_path, i, model_eval, debug=True)#debug=FLAGS.debug)
            if raw_dataset is not None:
                obs = jax.device_put(raw_dataset['observations'][::10])
                act = jax.device_put(raw_dataset['actions'][::10])
                q_dataset = agent.critic(obs, act); q_dataset = jax.device_get(q_dataset)
    ...
    score, length = [], []
    for i in range(10):
        eval_stats = evaluate(FLAGS.seed + i * FLAGS.eval_episodes, agent, eval_envs, video_path, 1000000, model_eval)
