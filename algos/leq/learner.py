...
import policy
import value_net
from algos.leq.actor import update_actor_bc, DPG_lambda_update_actor, DPG_multistep_update_actor, onestep_update_actor
from common import Batch, InfoDict, Model, PRNGKey, Params, get_deter
from algos.leq.critic import update_q_fqe, onestep_update_q, multistep_update_q, lambda_update_q
...
@jax.jit
def _update_fqe_jit(
        critic:Model, target_critic: Model, actor: Model, data_batch: Batch, discount: float, soft_target_update: float, 
    ) -> Tuple[Model, InfoDict]: 
    
    new_critic, critic_info = update_q_fqe(critic, target_critic, actor, data_batch, discount) 
...
@partial(jax.jit, static_argnames=['horizon_length', 'num_repeat', 'actor_update', 'critic_update'])
def _update_jit(
    rng: PRNGKey, actor: Model, critic: Model, target_critic: Model, model: Model, 
        data_batch: Batch, model_batch: Batch, model_batch_ratio: float, discount: float, soft_target_update: float,  
        expectile: float, lamb: float, horizon_length: int, num_repeat: int,
        actor_update: str, critic_update: str,
    ...
    if actor_update == 'lambda-return': 
        ...
        new_actor, actor_info = DPG_lambda_update_actor(key, actor, critic, model,
                                                     model_batch, discount, lamb, horizon_length, expectile, num_repeat)
    elif actor_update == 'multi-step':
        # Directly optimize lower-expectile of multi-step returns
        new_actor, actor_info = DPG_multistep_update_actor(key, actor, critic, model,
                                                           model_batch, discount, horizon_length, expectile, num_repeat)
    elif actor_update == 'one-step': 
        # Optimize Q(s,a)
        new_actor, actor_info = onestep_update_actor(key, actor, critic, model,
                                                     model_batch, discount, horizon_length, expectile, num_repeat)
    else:
    ...
    if critic_update == 'lambda-return':
        ...
        new_critic, critic_info = lambda_update_q(key3, critic, target_critic, actor, model,
                                           data_batch, model_batch, model_batch_ratio, 
                                           discount, lamb, horizon_length, expectile, num_repeat)
    elif critic_update == 'multi-step':
        # Use lower-expectile of multi-step returns as target value
        new_critic, critic_info = multistep_update_q(key3, critic, target_critic, actor, model,
                                           data_batch, model_batch, model_batch_ratio, 
                                           discount, horizon_length, expectile, num_repeat)
    elif critic_update == 'one-step':
        # Use lower-expectile of 1-step returns as target value
        new_critic, critic_info = onestep_update_q(key3, critic, target_critic, actor, model,
                                           data_batch, model_batch, model_batch_ratio,
                                           discount, horizon_length, expectile, num_repeat)
    else:
...
class Learner(object):
    def __init__(self,
        ...
        assert critic_update in ['lambda-return', 'multi-step', 'one-step'], critic_update
        ...
        actor_pretrain = Model.create(actor_def, inputs=[actor_key, observations], tx=optax.adam(learning_rate=actor_lr))
        actor_pretrain = _replace(actor_pretrain, {'scaler': obs_scaler})
        critic_pretrain = Model.create(critic_def, inputs=[critic_key, observations, actions], tx=optax.adam(learning_rate=value_lr))
        target_critic_pretrain = Model.create(critic_def, inputs=[critic_key, observations, actions], tx=optax.adam(learning_rate=value_lr))
    ...
    def rollout(self,
        ...
        with jax.transfer_guard('allow'):
            results = _rollout(key, observations, rollout_length, self.actor, self.model, temperature)
        results = {k: jax.device_get(v) for (k, v) in results.items()}
        return results
        
    def preprocess(self, batch):
        new_batch = Batch(observations=jax.device_put(batch.observations),
    ...
    def update_fqe(self, data_batch: Batch):
        data_batch = self.preprocess(data_batch)
        new_critic_pretrain, new_target_critic_pretrain, info = _update_fqe_jit(self.critic_pretrain, self.target_critic_pretrain, self.actor, data_batch, self.discount, self.soft_target_update)
    ...
    def update(self, data_batch: Batch, model_batch: Batch, model_batch_ratio: float) -> InfoDict:
        data_batch, model_batch = self.preprocess(data_batch), self.preprocess(model_batch)
        new_rng, new_actor, new_critic, new_target_critic, info = _update_jit(
            self.rng, self.actor, self.critic, self.target_critic, self.model,
            data_batch, model_batch, model_batch_ratio, self.discount, self.soft_target_update, 
            self.expectile, self.lamb, self.horizon_length, self.num_repeat,
            self.actor_update, self.critic_update)
