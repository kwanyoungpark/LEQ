...
def update_q_fqe(critic: Model, target_critic: Model, actor: Model, batch: Batch, discount: float):
    ...
    def critic_loss_fn(critic_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        return critic_loss + critic_reg_loss, {
            'critic_loss': critic_loss, 'critic_reg_loss': critic_reg_loss,
            'reward': batch.rewards.mean(), 'reward_min': batch.rewards.min(), 'reward_max': batch.rewards.max(),
            'q': q.mean(), 'q_min': q.min(), 'q_max': q.max(),
...
def onestep_update_q(key: PRNGKey, critic: Model, target_critic: Model, actor: Model, model: Model, 
             data_batch: Batch, model_batch: Batch, model_batch_ratio: float,
            discount: float, H: int, expectile: float, num_repeat: int) -> Tuple[Model, Model, InfoDict]:
    ...
    states, rewards, actions, masks, mask_weights, loss_weights = [Robs], [], [Ra], [], [jnp.ones(N*num_repeat)], [jnp.ones(N*num_repeat)]
    for i in range(H):
        s, a = states[-1], actions[-1]
        rng1, rng2, key1 = jax.random.split(key1, 3)
        s_next, rew, terminal, _ = model(rng2, s, jnp.clip(a, -1., 1.))
    ...
    target_q_rollout = []
    for i in range(H):
        target_q_rollout.append(rewards[i] + masks[i] * discount * critic(states[i+1], actions[i+1])) 
    ...
    def critic_loss_fn(critic_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        critic_reg_loss = critic_reg_loss_rollout * model_batch_ratio + critic_reg_loss_data * (1 - model_batch_ratio)
        return critic_loss_data + critic_loss_rollout + critic_reg_loss, {
            'critic_loss_data': critic_loss_data.mean(), 'critic_loss_model': critic_loss_rollout.mean(),
            ...
            'q_model': q_rollout.mean(), 'q_model_min': q_rollout.min(), 'q_model_max': q_rollout.max(), 
            'reward_model': (rewards * mask_weights[:-1]).mean(), 'reward_max': (rewards * mask_weights[:-1]).max(), 'reward_min': (rewards * mask_weights[:-1]).min(),
    ...
    infos = {
        'mask_weights': mask_weights.mean(), 'loss_weights': loss_weights.mean(),
        'expectile': expectile, 'model_batch_ratio': model_batch_ratio,
...
def multistep_update_q(key: PRNGKey, critic: Model, target_critic: Model, actor: Model, model: Model, 
             data_batch: Batch, model_batch: Batch, model_batch_ratio: float,
            discount: float, H: int, expectile: float, num_repeat: int) -> Tuple[Model, Model, InfoDict]:
    ...
    for i in reversed(range(H)):
        ...
        q_next = mask_weights[i] * rewards[i] + mask_weights[i+1] * discount * target_q_rollout[-1]
...
def lambda_update_q(key: PRNGKey, critic: Model, target_critic: Model, actor: Model, model: Model, 
             data_batch: Batch, model_batch: Batch, model_batch_ratio: float,
            discount: float, lamb: float, H: int, expectile: float, num_repeat: int) -> Tuple[Model, Model, InfoDict]:
