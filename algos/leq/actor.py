...
def update_actor_bc(actor: Model, batch: Batch) -> Tuple[Model, InfoDict]:
    ...
    def actor_loss_fn(actor_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        dist = actor.apply({'params': actor_params}, batch.observations, 0.0, training=True)
        a = get_deter(dist)
...
def onestep_update_actor(key: PRNGKey, actor: Model, critic: Model, model: Model,
        batch: Batch,
        discount: float, H: int, expectile: float, num_repeat: int) -> Tuple[Model, InfoDict]:
    ...
    def calculate_gae_fwd(Robs, Ra, key0):
        states, rewards, actions, mask_weights, keys = [Robs], [], [Ra], [1.], [key0]
        for i in range(H):
    ...
    def actor_loss_fn(actor_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        return actor_loss, {'actor_loss': actor_loss,
                            'policy_std': (policy_std * mask_weights0[:, :, None]).mean() / mask_weights0.mean(),
...
def DPG_lambda_update_actor(key: PRNGKey, actor: Model, critic: Model, model: Model,
        batch: Batch,
        discount: float, lamb: float, H: int, expectile: float, num_repeat: int) -> Tuple[Model, InfoDict]:
    ...
    def calculate_gae_fwd(Robs, Ra, key0):
        ...
        q_rollout, lamb_weight = [q_values[-1]], 1. 
        for i in reversed(range(H)):
            q_next = mask_weights[i] * rewards[i] + mask_weights[i+1] * discount * q_rollout[-1]
            next_value = (q_values[i] + lamb * lamb_weight * q_next) / (1 + lamb * lamb_weight)
        ...
        ep_weights = []
        for i in range(H):
            ep_weights.append(jnp.where(q_rollout[i] > q_values[i], expectile, 1-expectile))
    ...
    keys = jax.random.split(key, N*num_repeat)
    vmap_fwd = lambda func: jax.vmap(func, in_axes=0, out_axes=1)
    states0, actions0, mask_weights0, q_rollout, ep_weights = vmap_fwd(calculate_gae_fwd)(Robs, Ra, keys)

    def calculate_gae_bwd(delta_a, Robs, Ra, key0):
        ## Generate imagined trajectory (identical with fwd)
        states, rewards, actions, mask_weights, keys = [Robs], [], [Ra+delta_a[0]], [1.], [key0]
    ...
    def actor_loss_fn(actor_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        actor_loss = -(ep_weights[:, :, None] * grad_gae * actions_grad).mean(axis=1).sum()
...
def DPG_multistep_update_actor(key: PRNGKey, actor: Model, critic: Model, model: Model,
        batch: Batch,
        discount: float, H: int, expectile: float, num_repeat: int) -> Tuple[Model, InfoDict]:
