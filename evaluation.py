...
@jax.jit
def step_imagine(key: PRNGKey, model_eval: Model, obs: jnp.ndarray, action: jnp.ndarray, states: jnp.ndarray):
...
def step_imagine_first(key: PRNGKey, model_eval: Model, obs: jnp.ndarray, action_dim: int, state_dim: int):
...
def evaluate(seed: int, agent: nn.Module, envs: List[gym.Env], video_path: str, step:int, model_eval=None, debug=False) -> Dict[str, float]:
    stats = {'return': [], 'length': []} 
    ...
    for j in tqdm(range(10000)):
        ...
        if model_eval is not None:
            ...
            if j == 0:
                ...
                _states = step_imagine_first(key, model_eval, _observations, action_dim, state_dim)
            else:
                _states = step_imagine(key, model_eval, _observations, _actions, jax.device_put(_states))
    ...
    if debug:
        ...
        q_values = agent.critic(_states, _actions); q_values = jax.device_get(q_values)
